...
#'
#' \code{oAuxEmbed} is supposed to encode a file as a base64 string for embedding such as into the R markdown rendering html file. It returns a hyperlink.
#'
...
#' @export
#' @seealso \code{\link{oAuxEmbed}}
#' @include oAuxEmbed.r
#' @examples
#' # file <- system.file("DESCRIPTION",package="Fang")
#' # res <- oAuxEmbed(file)

oAuxEmbed <- function(file, download.attribute=basename(file), link.text=paste0("Download ",download.attribute))
{
